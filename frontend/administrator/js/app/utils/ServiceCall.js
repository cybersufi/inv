Ext.define('App.utils.ServiceCall', {
	...
	*/
	...
	doCall: function () {
		...
		App.getApplication().getStatusbar().startLoad(this.loadMessage);
		...
		if (!this.anonymous) {
			headers.session = App.getApplication().getSessionManager().getSession();
		}
		
		Ext.Ajax.request({
			url: App.getBasePath() + '/' + this.service + "/"+this.call,
	...
	onSuccess: function (responseObj, options) {
		App.getApplication().getStatusbar().endLoad();
		...
		if (response.status == "error") {
			...
			App.getApplication().getStatusbar().setStatus({
