 ...
 */
Ext.define("App.views.BaseComponents.Grid.GridMenu", {
	...
	init: function(grid) {
		...
		this.menu = new Ext.menu.Menu({
			...
			items: [{
				text: "Export",
				icon: App.getResource('resources/fugue-icons/icons/application-export.png'),
				menu: [{
					icon: App.getResource('resources/mimetypes/csv.png'),
					text: 'Export as CSV (.csv)',
					handler: this.exportCSV,
					scope: this
				},{
					icon: App.getResource('resources/fugue-icons/icons/blue-document-excel.png'),
					text: 'Export as Excel XML (.xlsx)',
					handler: this.exportXLSX,
					scope: this
				},{
					icon: App.getResource('resources/icons/mediawiki_icon.png'),
	...
	doExport: function (data, filename) {
		var call = new App.utils.ServiceCall("TempFile", "jsonUpload");
